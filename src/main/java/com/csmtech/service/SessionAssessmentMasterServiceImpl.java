...
public class SessionAssessmentMasterServiceImpl implements SessionAssessmentMasterService {
	...
	/*
	 * @Override public ResponseEntity<?> getQuestionarBySessionId(Integer
	 * sessionId) { SessionAssessmentSetting sessionSessionAssessmentSetting =
	 * assessmentSettingRepository .findFirst1BySessionMaster_SessionId(sessionId);
	 * 
	 * Integer noOfQuestion = sessionSessionAssessmentSetting.getNumberOfQuestion();
	 * System.err.println(sessionId + " " + noOfQuestion);
	 * List<SessionAssessmentMaster> assessmentMastersList =
	 * sessionAssessmentMasterRepository .getQuestionarBySessionId(sessionId,
	 * noOfQuestion);
	 * 
	 * if (assessmentMastersList.isEmpty()) { return
	 * ResponseEntity.status(HttpStatus.NOT_FOUND).
	 * body("No questions found for session ID: " + sessionId); } else { return
	 * ResponseEntity.ok(assessmentMastersList); } }
	 */
